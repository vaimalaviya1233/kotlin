...
class ObjectFactoryStorage : private Pinned {
    ...
    public:
        void operator()(T* instance) noexcept {
            auto totalSize = instance->allocatedSize();
            instance->~T();
            Allocator::Free(instance, totalSize);
        }
    };
...
public:
    ...
    public:
        ...
        size_t allocatedSize() noexcept {
            auto dataSize = DataSizeProvider::GetDataSize(Data());
            auto totalSize = GetSizeForDataSize(dataSize);
            RuntimeAssert(totalSize <= std::numeric_limits<size_t>::max(), "Cannot have allocated more than size_t max size, but actual total size is %" PRIu64, totalSize);
            return totalSize;
        };
...
class ObjectFactory : private Pinned {
    ...
    struct DataSizeProvider {
        static uint64_t GetDataSize(void* data) noexcept {
            auto& object = static_cast<HeapObjHeader*>(data)->object;
