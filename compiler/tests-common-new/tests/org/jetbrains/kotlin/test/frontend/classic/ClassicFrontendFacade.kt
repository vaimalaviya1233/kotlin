    ...
    private class CommonDependenciesContainerImpl(module: ModuleDescriptor) : CommonDependenciesContainer {
        private class ModuleInfoImpl(val module: ModuleDescriptor) : ModuleInfo {
            ...
            override fun dependencies(): List<ModuleInfo> =
                listOf(this) + module.allDependencyModules.map(::ModuleInfoImpl) // todo return self?
        ...
        private val _moduleInfos: List<ModuleInfoImpl> = // todo return only self?
        ...
        override fun moduleDescriptorForModuleInfo(moduleInfo: ModuleInfo): ModuleDescriptor =
            _moduleInfos.singleOrNull { it.name == moduleInfo.name }?.module ?: error("fuck")

        override fun registerDependencyForAllModules(moduleInfo: ModuleInfo, descriptorForModule: ModuleDescriptorImpl) = Unit
        override fun packageFragmentProviderForModuleInfo(moduleInfo: ModuleInfo): PackageFragmentProvider? =
            (moduleInfo as? ModuleInfoImpl)?.module?.let { it as ModuleDescriptorImpl }?.packageFragmentProvider
