...
class IrCompileTimeChecker(
    ...
    override fun visitCall(expression: IrCall, data: Nothing?): Boolean {
        ...
        // This check must be placed here instead of CallInterceptor because we still want to evaluate `const val` expressions
        if (owner.name.asString() == "toFloat" && interpreterConfiguration.treatFloatInSpecialWay && visitedStack.none { it is IrProperty && it.isConst }) {
