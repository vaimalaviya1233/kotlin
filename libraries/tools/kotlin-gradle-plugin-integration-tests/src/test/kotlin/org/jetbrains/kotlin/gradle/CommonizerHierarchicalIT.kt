...
open class CommonizerHierarchicalIT : KGPBaseTest() {
    ...
    fun testCommonizeHierarchicallyMetadataCompilations(gradleVersion: GradleVersion) {
        nativeProject("commonizeHierarchically", gradleVersion) {
            ...
            build(":p1:compileMingwMainKotlinMetadata") {
                assertFileInProjectExists("p1/build/classes/kotlin/metadata/mingwMain/klib/p1_mingwMain.klib")
            }
    ...
    fun testCommonizeHierarchicallyKlibraryCompilations(gradleVersion: GradleVersion) {
        nativeProject("commonizeHierarchically", gradleVersion) {
            ...
            build(":p1:mingwX64MainKlibrary", ":p1:mingwX86MainKlibrary") {
                ...
                assertFileInProjectExists("p1/build/classes/kotlin/mingwX86/main/klib/p1.klib")
                assertNoDuplicateLibraryWarning()
            }
        }
    }
    ...
    fun testIsolatedJvmSubprojectShouldNotFailCommonization(gradleVersion: GradleVersion) {
        nativeProject(
            "commonize-kt-50592-withIsolatedJvmSubproject",
            gradleVersion = gradleVersion,
            buildOptions = defaultBuildOptions.copy(enableAndroidSdk = false) // Necessary to ensure separated classpath
        ) {
            build("commonize") {
                assertTasksExecuted(":commonizeNativeDistribution")
            }
